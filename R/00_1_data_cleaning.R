...
data2 <- data1[!is.na(naics_sector)]
n2 <- nrow(data2)

data3 <- data2[!is.na(wage)]
n3 <- nrow(data3)

drop_summary <- data.frame(
  step = c("original", "drop NA czone", "drop NA naics_sector", "drop NA wage"),
  n_obs = c(n0, n1, n2, n3),
  dropped_from_previous = c(0, n0 - n1, n1 - n2, n2 - n3),
  dropped_total = c(0, n0 - n1, n0 - n2, n0 - n3)
)

write.csv(drop_summary,
          file = "plots/descriptive_statistics/drop_summary.csv",
          row.names = FALSE)
